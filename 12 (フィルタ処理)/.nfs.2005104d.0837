...
void processing(){
    ...
    for(int j = 0; j < height; j++){
        for(int k = 0; k < width; k++){
            ...
            double insertion[2] = {(dx - (int)dx), (dy - (int)dy)};
            double min_ins[2] = {(1 - insertion[0]), (1 - insertion[1])};
            ...
            if(((0 <= x) && (x < width - 1)) && ((0 <= y) && (y < height - 1))){
                ...
                for (int i = 0; i < 3; i++){
                    for(int l = 0; l < 2; l++){
                        tmp[l] = (imgin[i][y + l][x] * min_ins[0]) + (imgin[i][y + l][x + 1] * insertion[0]);
                    }
                    result = (int)((tmp[0] * min_ins[1]) + (tmp[1] * insertion[1]));
                    imgout[i][j][k] = result;
                }
            }else{
                imgout[0][j][k] = 0;
