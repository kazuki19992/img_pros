...
void processing(){
    ...
    for(int i = 0; i < 2; i++){
        ...
        // 内挿点入力
        scanf("%lf", &insertion[i]);

        // 不正値検出
        if(insertion[i] < 0 || insertion[i] >= 1 ){
            i--;
            continue;
        }
    }
    ...
    for (int i = 0; i < 2; i++){
        // 横
        tmp[i] = (d[0][i] * min_ins[0]) + (d[1][i] * insertion[0]);
    }

    result = (int)((tmp[0] * min_ins[1]) + (tmp[1] * insertion[1]) + 0.5);
