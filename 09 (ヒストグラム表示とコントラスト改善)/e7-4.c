...
void processing(void){
    ...
    for(i=0;i<height;i++){
        for(j=0;j<width;j++){
            imgout[0][j][i]=(255/(max-min))*(imgin[0][j][i]-min);
            /* k=0;
            while(k<=255){
                if(imgout[0][j][i]==k){
                    count[k]++;
    ...
    /*for(k=0;k<=255;k++){
        count_x[k] = (double)count[k]/(double)max;
        count_x[k] = count_x[k]*100;
        count_x[k] = count_x[k]+0.5;
        }*/
